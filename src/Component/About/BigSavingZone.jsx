        ...
        <div className="flex w-full justify-around overflow-hidden mt-8 px-2">
          {data.slice(currentIndex, currentIndex + cardsToShow).map((item, index) => (
            ...
            >
              ...
              >
                <div className="absolute bottom-2 left-2 right-2 bg-yellow-500 bg-opacity-80 p-2 text-white text-sm font-bold rounded-md">
                  <span className="block text-lg md:text-xl font-semibold">{item.title}</span>
                  <span className="block text-xs md:text-sm text-black">
                    Choose Two at <br />
                    <button className='font-bold hover:bg-slate-100 p-1 rounded-sm hover:text-green-500 bg-green-500 text-white mt-2 md:mt-6'>
